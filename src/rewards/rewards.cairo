...
pub mod Rewards {
    ...
    struct Storage {
        ...
        player_cycle_rewards: Map<
            (ContractAddress, u32), u256,
        >, // (player, cycle_id) -> reward_amount
    ...
    impl RewardsImpl of IRewards<ContractState> {
        fn claim_reward(ref self: ContractState, cycle_id: u32) -> u256 {
            ...
            let leaderboard = ILeaderboardDispatcher {
                contract_address: self.leaderboard_contract.read(),
            };
            ...
            self
                .emit(
                    RewardClaimed {
                        player: caller, cycle_id, tier_name, amount: reward_amount, rank,
                    },
                );
        ...
        fn get_claimable_reward(
            self: @ContractState, player: ContractAddress, cycle_id: u32,
        ) -> u256 {
            // Check if cycle exists and is finalized
            let cycle = self.challenge_cycles.read(cycle_id);
        ...
        fn create_challenge_cycle(
            ref self: ContractState, start_time: u64, end_time: u64, total_pool: u256,
        ) -> u32 {
            self._assert_owner();
            ...
            self.emit(ChallengeCycleCreated { cycle_id, start_time, end_time, total_pool });
        ...
        fn finalize_challenge_cycle(ref self: ContractState, cycle_id: u32) {
            ...
            self
                .emit(
                    ChallengeCycleFinalized {
                        cycle_id, participants: total_players, total_distributed: cycle.total_pool,
                    },
                );
        }
        ...
        fn calculate_tier_rewards(
            self: @ContractState, cycle_id: u32, player: ContractAddress,
        ) -> u256 {
            let cycle = self.challenge_cycles.read(cycle_id);
            if cycle.cycle_id == 0 {
                return 0;
