...
pub trait ITreasury<TContractState> {
    // Fee Collection
    fn collect_fee(
        ref self: TContractState, fee_type: felt252, amount: u256, payer: ContractAddress,
    );
    ...
    fn withdraw_from_pool(
        ref self: TContractState, pool_type: felt252, amount: u256, recipient: ContractAddress,
    );
    fn transfer_between_pools(
        ref self: TContractState, from_pool: felt252, to_pool: felt252, amount: u256,
    );

    // Fee Configuration
    fn set_fee_rate(
        ref self: TContractState, operation: felt252, rate_bps: u16,
    ...
    fn set_authorized_contract(
        ref self: TContractState, contract_address: ContractAddress, authorized: bool,
    );
