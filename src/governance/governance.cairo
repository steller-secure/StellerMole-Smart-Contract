...
pub mod DAOGovernance {
    use starknet::{
        ContractAddress, ClassHash, get_caller_address, get_block_timestamp, syscalls::call_contract_syscall
    };
    ...
    impl GovernanceImpl of IGovernance<ContractState> {
        ...
        fn execute(ref self: ContractState, proposal_id: u256) {
            ...
            assert(
                current_time >= proposal.queue_time + config.timelock_delay,
    ...
    impl InternalImpl of InternalTrait {
        fn _get_proposal_state_internal(self: @ContractState, proposal_id: u256) -> u8 {
            ...
            if proposal.for_votes > proposal.against_votes {
                if proposal.queued {
                    let config = self.config.read();
                    if current_time > proposal.queue_time + config.timelock_delay + (7 * 24 * 60 * 60) { // 7 days grace period
