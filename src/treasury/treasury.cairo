#[starknet::contract]
pub mod Treasury {
    use starkmole::interfaces::treasury::{ITreasury};
    use starkmole::types::{
        PoolTypes, FeeTypes, PoolInfo, WithdrawalProposal, FeeRecord, FeeCollection,
        FeeDistribution,
    };
    ...
    struct Storage {
        ...
        proposal_approvals: Map<
            (u32, ContractAddress), bool,
        >, // (proposal_id, approver) -> approved
        proposal_count: u32,
    ...
    impl TreasuryImpl of ITreasury<ContractState> {
        fn collect_fee(
            ref self: ContractState, fee_type: felt252, amount: u256, payer: ContractAddress,
        ) {
            ...
            self
                .emit(
                    FeeCollected {
                        fee_type, amount, payer, collector: caller, timestamp: current_time,
                    },
                );
        }
        ...
        fn deposit_to_pool(ref self: ContractState, pool_type: felt252, amount: u256) {
            ...
            self
                .pool_total_inflows
                .write(pool_type, self.pool_total_inflows.read(pool_type) + amount);
        ...
        fn withdraw_from_pool(
            ref self: ContractState, pool_type: felt252, amount: u256, recipient: ContractAddress,
        ) {
            self._assert_admin();
            self._assert_pool_exists(pool_type);
            ...
            self
                .pool_total_outflows
                .write(pool_type, self.pool_total_outflows.read(pool_type) + amount);
            self.total_treasury_value.write(self.total_treasury_value.read() - amount);
        ...
        fn transfer_between_pools(
            ref self: ContractState, from_pool: felt252, to_pool: felt252, amount: u256,
        ) {
            self._assert_admin();
            ...
            self
                .pool_total_outflows
                .write(from_pool, self.pool_total_outflows.read(from_pool) + amount);
        ...
        fn set_fee_distribution(ref self: ContractState, distributions: Array<FeeDistribution>) {
            ...
            while j < old_count {
                self
                    .fee_distributions
                    .write(j, FeeDistribution { pool_type: 0, percentage_bps: 0 });
                j += 1;
            };
        ...
        fn propose_withdrawal(
            ...
            self
                .emit(
                    WithdrawalProposed {
                        proposal_id, pool_type, amount, recipient, proposer: caller,
                    },
                );
        ...
        fn approve_withdrawal(ref self: ContractState, proposal_id: u32) {
            ...
            self
                .emit(
                    WithdrawalApproved {
                        proposal_id, approver: caller, approval_count: proposal.approval_count,
                    },
                );
        }
        ...
        fn execute_withdrawal(ref self: ContractState, proposal_id: u32) {
            ...
            assert(!proposal.executed, 'Proposal already executed');
            assert(
                proposal.approval_count >= proposal.required_approvals, 'Insufficient approvals',
            );
            ...
            self
                ...
                .write(
                    proposal.pool_type,
                    self.pool_total_outflows.read(proposal.pool_type) + proposal.amount,
                );
            ...
            self
                .emit(
                    WithdrawalExecuted {
                        proposal_id, amount: proposal.amount, recipient: proposal.recipient,
                    },
                );
        }
        ...
        fn get_fee_collection_history(
            ...
            let end = if start + limit > total_records + 1 {
                total_records + 1
            } else {
                start + limit
            };
        ...
        fn emergency_withdraw_all(ref self: ContractState, recipient: ContractAddress) {
            ...
            if balance > 0 {
                ...
                self
                    .emit(
                        EmergencyWithdrawal { amount: balance, recipient, authorized_by: caller },
                    );
            }
        }
    ...
    impl InternalImpl of InternalTrait {
                ...
                if pool_amount > 0 {
                    ...
                    self
                        ...
                        .write(
                            ...
                            self.pool_total_inflows.read(distribution.pool_type) + pool_amount,
                        );
                }
        ...
        fn _initialize_default_fee_distribution(ref self: ContractState) {
            // Set default fee distribution: 50% rewards, 20% DAO, 15% dev, 10% marketing, 3%
            // liquidity, 2% insurance
            self
                .fee_distributions
                .write(
                    0, FeeDistribution { pool_type: PoolTypes::REWARDS_POOL, percentage_bps: 5000 },
                );
            self
                .fee_distributions
                .write(1, FeeDistribution { pool_type: PoolTypes::DAO_POOL, percentage_bps: 2000 });
            self
                .fee_distributions
                .write(2, FeeDistribution { pool_type: PoolTypes::DEV_FUND, percentage_bps: 1500 });
            self
                .fee_distributions
                .write(
                    3,
                    FeeDistribution { pool_type: PoolTypes::MARKETING_POOL, percentage_bps: 1000 },
                );
            self
                .fee_distributions
                .write(
                    4,
                    FeeDistribution { pool_type: PoolTypes::LIQUIDITY_POOL, percentage_bps: 300 },
                );
            self
                .fee_distributions
                .write(
                    5,
                    FeeDistribution { pool_type: PoolTypes::INSURANCE_POOL, percentage_bps: 200 },
                );
