...
pub mod ChallengeScheduler {
    ...
    struct Storage {
        ...
        challenge_participant_list: Map<
            (u32, u32), ContractAddress,
        >, // (challenge_id, index) -> participant
    ...
    impl ChallengeSchedulerImpl of IChallengeScheduler<ContractState> {
        fn create_challenge(
            ...
            assert!(max_participants > 0, "Max participants must be greater than 0");
        ...
        fn cancel_challenge(ref self: ContractState, challenge_id: u32) {
            ...
            self.emit(ChallengeCancelled { challenge_id, cancelled_by: get_caller_address() });
        }
        ...
        fn join_challenge(ref self: ContractState, challenge_id: u32) {
            ...
            assert!(
                ...
                "Challenge is not in valid time window",
            );
            assert!(challenge.participant_count < challenge.max_participants, "Challenge is full");
            assert!(
                !self.participant_exists.read((challenge_id, participant)),
                "Already participating in challenge",
            );
            ...
            self.emit(ParticipantJoined { challenge_id, participant, joined_at: current_time });
        }
        ...
        fn leave_challenge(ref self: ContractState, challenge_id: u32) {
            ...
            self.emit(ParticipantLeft { challenge_id, participant });
        }
        ...
        fn submit_score(ref self: ContractState, challenge_id: u32, score: u128) {
            ...
            let mut participant_data = self
                .challenge_participants
                .read((challenge_id, participant));
            participant_data.score = score;
            self.challenge_participants.write((challenge_id, participant), participant_data);

            self.emit(ScoreSubmitted { challenge_id, participant, score });
        }
        ...
        fn get_active_challenges(self: @ContractState) -> Array<Challenge> {
            ...
            while i < active_count {
                ...
                if current_time >= challenge.start_time
        ...
        fn get_challenge_participants(
            self: @ContractState, challenge_id: u32,
            ...
            while i < challenge.participant_count {
                ...
                let participant_data = self
                    .challenge_participants
                    .read((challenge_id, participant_address));
        ...
        fn is_participant(
            self: @ContractState, challenge_id: u32, participant: ContractAddress,
        ) -> bool {
            self.participant_exists.read((challenge_id, participant))
        }
        ...
        fn set_game_contract(ref self: ContractState, game_contract: ContractAddress) {
            ...
            self.emit(ContractUpdated { contract_type: 'game', new_address: game_contract });
        }

        fn set_leaderboard_contract(
            ref self: ContractState, leaderboard_contract: ContractAddress,
        ) {
            self.assert_only_owner();
            self.leaderboard_contract.write(leaderboard_contract);
            ...
            self
                .emit(
                    ContractUpdated {
                        contract_type: 'leaderboard', new_address: leaderboard_contract,
                    },
                );
        }
