...
fn deploy_treasury_and_token() -> (
    ITreasuryDispatcher, ContractAddress, IERC20Dispatcher, IMockERC20Dispatcher, ContractAddress,
) {
...
fn test_fee_distribution_configuration() {
    ...
    new_distributions
        .append(
            FeeDistribution { pool_type: PoolTypes::REWARDS_POOL, percentage_bps: 7000 },
        ); // 70%
    new_distributions
        .append(FeeDistribution { pool_type: PoolTypes::DAO_POOL, percentage_bps: 3000 }); // 30%
...
fn test_fee_collection() {
    let (treasury_contract, treasury_address, token_contract, mock_token, token_address) =
        deploy_treasury_and_token();
    let owner = contract_address_const::<'treasury_owner'>();
    let collector = contract_address_const::<'fee_collector'>();
...
fn test_pool_transfer() {
    ...
    treasury_contract
        .transfer_between_pools(PoolTypes::REWARDS_POOL, PoolTypes::DAO_POOL, transfer_amount);
...
fn test_multisig_withdrawal_proposal() {
    ...
    let proposal_id = treasury_contract
        .propose_withdrawal(PoolTypes::REWARDS_POOL, withdrawal_amount, recipient);
...
fn test_execute_withdrawal_insufficient_approvals() {
    ...
    start_cheat_caller_address(treasury_address, admin1);
    let proposal_id = treasury_contract
        .propose_withdrawal(PoolTypes::REWARDS_POOL, 1000, recipient);
    treasury_contract.approve_withdrawal(proposal_id);
    stop_cheat_caller_address(treasury_address);
