...
fn setup_contracts() -> (IReferralDispatcher, IStarkMoleGameDispatcher, ContractAddress, ContractAddress) {
    ...
    let referral_constructor_calldata = array![owner.into(), game_address.into(), treasury.into(), token.into()];
...
fn test_full_referral_flow_with_game() {
    ...
    let code = 'GAMEREF';
    referral.create_referral_code(code, 10, get_block_timestamp() + 172800); // 2 days to account for time advance
    ...
    start_cheat_block_timestamp(referral.contract_address, get_block_timestamp() + 86401); // 24 hours + 1 second
